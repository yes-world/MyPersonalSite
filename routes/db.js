                ...
                if (results.length > 0) {
                    collection.deleteOne({ email: email }, function (err, result) {
                        ...
                        collection.insertOne(user, function (err, result) {
                            ...
                            req.session.message = "На указанный электронный адрес отправлено письмо с подтверждением. Перейдите по ссылке в письме, чтобы войти в профиль. Могут наблюдаться задержки доставки писем, а также попаданием их в спам. Пожалуйста, проверьте папку спам или дождитесь письма.";
                            req.session.save(() => res.redirect('/login'));
                        });
                    });
                } else {
...
function passwordRecovery(req, res) {
    ...
    collection.find({ email: email }).toArray(function (err, results) {
        ...
        if (!results.length) {
            ...
            req.session.message = "На указанный электронный адрес выслано письмо с ссылкой на восстановление пароля. Могут наблюдаться задержки доставки писем, а также попаданием их в спам. Пожалуйста, проверьте папку спам или дождитесь письма.";
            req.session.save(() => res.redirect('/login'));
        }
    });
}
...
function feedback(req, res) {
    ...
    collection.insertOne(message, function (err, result) {
        if (err) return console.log(err);
        sendEmail('d.sabitov@mail.ru', "Получена обратная связь", '<html style="font-family: sans-serif;"><body style="margin: 0;"><div style="width: 100%; height: 3rem; background-color: #910083; color: white; padding-left: 3rem; padding-top: 1.25rem;"><div>yes.world</div></div><div style="margin-top: 3rem; margin-left: 3rem;"><p>' + message.date + '</p><p>' + message.author + '</p><p>' + message.email + '</p><p>' + message.subject + '</p><p>' + message.message + '</p></div></body></html>').catch(console.error);
